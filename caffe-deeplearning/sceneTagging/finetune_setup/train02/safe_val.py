#Testing file for 'safe' tags, compared to MIT streetscores
...
model_def = caffe_root + 'models/finetune_scene/train02/deploy.prototxt'
model_weights = caffe_root + 'models/finetune_scene/train02/_iter_100000.caffemodel'

# model_def = caffe_root + 'Carmera-SceneDetection/createTrain/finetune/train02/deploy.prototxt'
# model_weights = caffe_root + 'Carmera-SceneDetection/createTrain/finetune/train02/_iter_100000.caffemodel'
...
results_file = 'sComparison.txt'
errors_file = 'sErrors.txt'
network_output_file = 'sNetworkOutput.txt'

format_results = '%-25s %10s %10s %10s %10s\n'
format_error = '%-25s %40s %40s\n'
format_network = '%-25s %25s %25s\n'

fResults = open(results_file, 'a')
fResults.write(format_results % ('Long,Lat', 'Streetscore', 'Network', 'Match', 'Radius'))
fErrors = open(errors_file, 'a')
fErrors.write(format_error % ('Long,Lat', 'Error', 'Error Message'))
fNetwork = open(network_output_file, 'a')
fNetwork.write(format_network % ('Long,Lat', 'Image Ids', 'Network Safety Scores'))

cutOffImages = 15
data_location = 'data/scene/images2/'

# def writeNetworkOutput(coordinates, image_ids, network_analysis):
# 	fNetwork.write(format_network % 
# 		(coordinates, str(image_ids).strip('[]'), str(network_analysis).strip('[]')))
...
def chooseImages(coordinates):
	...
	#randomly choose num(cutOffImages) images from data
	#return these images
		image_ids = []
	for feature in data['features']:
		image_ids.append(feature['properties']['id'])
	random.shuffle(image_ids)
	image_ids = image_ids[:cutOffImages]
	image_ids.append(current_radius - inc_radius)
	return image_ids
	...
	for image_id in images:
		...
		try:
			...
			fErrors.write(format_error % 
					  (str(image_id), 'Unable to download following image', str(e)))
...
#---------------------------------------------------------------------------------------------
#Run through network
def runThroughNetwork(coordinates, images, cutOffValue):
	...
	for image_id in images:
		...
		if (not os.path.isfile(file_exists)):
			...
			fErrors.write(format_error % 
					  (str(image_id), 'The image disappeared or was never downloaded', 
									   'coordinate tested on {} images'.format(cutOffImages - num_images)))
			continue
		...
		try:
			...
			fNetwork.write(format_network % (coordinates, image_id, safe_prob))
			...
			fErrors.write(format_error %
				(str(image_id), 
				'Image unable to be analyzed, tested on {} images'.format(cuffOffImages - num_images),
				str(e)))
...
with open(verify_file) as csvfile: #Creates csv file object
	...
	for row in reader:
		...
		if qscore < 4.5:
			...
			fErrors.write(format_error % (coordinates, 'Indecisive qscore', 'Skipped this coordinate'))
			continue
		...
		if image_ids == None:
			...
			fErrors.write(format_error % 
					  (coordinates, 'Less than 10 images in 150m radius', 'Not enough data at coordinate'))
			continue
				
		radius_size = image_ids.pop()
		...
		network_safe = runThroughNetwork(coordinates, image_ids, .01)
		fResults.write(format_results % 
			(coordinates, hot_safe, network_safe, (hot_safe==network_safe), radius_size))
