...
file_path = 'Carmera-SceneDetection/sceneTagging/finetune_results/train02/'
...
model_weights = caffe_root + file_path + '_iter_100000.caffemodel'
...
dictTest = {'133228': 'Desolate',
            '146202': 'Safe, Family Friendly',
            '162005': 'Safe, Vibrant',
            '199691': 'Desolate',
            '299650': 'Desolate'}

#Tags for the first network, in order of output nodes
tags = ['Safe', 'Green', 'Family Friendly', 'Beautiful',
    'Desolate', 'Vibrant']
...
for im in listTest[0:5]:
    ...
    #Need to remove last two elements in array (accidently trained with 8 output nodes)
    np.delete(output_prob, [6,7])

    labels_file = caffe_root + file_path + 'label_names.txt'
    labels = np.loadtxt(labels_file, str, delimiter='\t')
    # sort top five predictions from softmax output
    top_inds = output_prob.argsort()[::-1][:4]  # reverse sort and take 5 largest items
